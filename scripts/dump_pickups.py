"""Generate the world pickup table from a clean VC decompile.

The randomize_pickups option permutes the world pickups: street
...
- every allowlisted creation in the Mission 0 block, the init mission that
  places the world items at a new game, and
...
world slots like any other. The four standing in Phil's Place are in-shop
...
# Every world creation stores its pickup handle in a global, which is the
...
MODULE_DOCSTRING = '''"""World pickup data, generated by scripts/dump_pickups.py.
...
def extract(lines: list[str]) -> tuple[
    ...
    handles: list[int] = []
    # The permanent mission creations, kept apart from the world walk above so
