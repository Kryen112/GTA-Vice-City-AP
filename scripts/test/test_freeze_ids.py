"""Tests for the id snapshot writer's refusals.
...
def test_matching_tables_have_moved_nothing() -> None:
    assert freeze_ids.moved_entries(_snapshot(), TABLES) == []
...
def test_a_released_snapshot_missing_a_table_is_reported() -> None:
    ...
    without = _snapshot()
    del without["items"]
    assert freeze_ids.moved_entries(without, TABLES) == [
...
def _run(tmp_path: pathlib.Path, monkeypatch, argv: list[str],
    ...
    printed: list[str] = []
    monkeypatch.setattr(freeze_ids, "SNAPSHOT", path)
    monkeypatch.setattr(freeze_ids, "REPOSITORY_ROOT", tmp_path)
    monkeypatch.setattr(freeze_ids, "archipelago_root", lambda: tmp_path)
    monkeypatch.setattr(freeze_ids, "link_world", lambda root: tmp_path)
    monkeypatch.setattr(freeze_ids, "current_tables", lambda: dict(TABLES))
    monkeypatch.setattr(freeze_ids, "print", lambda *args: printed.append(" ".join(map(str, args))), raising=False)
    monkeypatch.setattr(sys, "argv", ["freeze_ids.py", *argv])
    return freeze_ids.main(), "\n".join(printed), path
