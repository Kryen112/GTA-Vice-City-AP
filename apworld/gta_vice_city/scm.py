...
- item_effects: AP item id -> a one-shot effect descriptor. The ASI applies
  each consumable (cash, weapon, health, armor) once past the saved applied-index.
...
def item_effects() -> dict[int, list]:
    ...
    the ASI past the saved applied-index."""
    return {
        items.ITEM_NAME_TO_ID[name]: [effect[0], *effect[1:]]
        for name, effect in data.CONSUMABLE_EFFECTS.items()
    }
