...
// Based on APCpp by N00byKing and contributors; C port by randomcodegen.
// Copyright (c) 2024-2026 randomcodegen, for the C port and modifications.
// Modified 2026-09-07: packet callbacks, locking, TLS and transport handling.
...
static void (*packet_callback)(json_t*) = NULL;
...
static int lws_callbacks(struct lws* wsi, enum lws_callback_reasons reason, void* user, void* in, size_t len)
{
    ...
    case LWS_CALLBACK_CLIENT_RECEIVE:
        ...
        if (psd->message_buffer) {
            if (psd->message_buffer->len + len > 16u * 1024u * 1024u) return -1;
            if (len) g_string_append_len(psd->message_buffer, (const gchar*)in, len);
            if (!lws_is_final_fragment(wsi) || lws_remaining_packet_payload(wsi)) break;
            json_t* json = json_loadb(psd->message_buffer->str, psd->message_buffer->len, 0, &jerror);
            if (json) {
                if (packet_callback) packet_callback(json);
                else parse_response(json);
                json_decref(json);
    ...
    case LWS_CALLBACK_CLIENT_WRITEABLE: {
        AP_LOCK();
        json_t* packet = outgoing_queue ? g_queue_pop_head(outgoing_queue) : NULL;
        AP_UNLOCK();
        if (!packet) break;
        char* text = json_dumps(packet, JSON_COMPACT);
        json_decref(packet);
        if (!text) return -1;
        size_t length = strlen(text);
        unsigned char* buffer = malloc(LWS_PRE + length);
        if (!buffer) { free(text); return -1; }
        memcpy(buffer + LWS_PRE, text, length);
        int written = lws_write(wsi, buffer + LWS_PRE, length, LWS_WRITE_TEXT);
        free(text);
        free(buffer);
        if (written < 0 || (size_t)written < length) return -1;
        AP_LOCK();
        bool more = outgoing_queue && !g_queue_is_empty(outgoing_queue);
        AP_UNLOCK();
    ...
    case LWS_CALLBACK_CLIENT_CONNECTION_ERROR:
        ...
        if (!packet_callback) isSSL = !isSSL;
        AP_UNLOCK();
        break;
...
void AP_WebService() {
    ...
    if (!web_socket)
    {
        ...
        if (isSSL) ccinfo.ssl_connection = LCCSCF_USE_SSL;
...
void AP_Init(const char* ip, int port, const char* game, const char* player_name, const char* passwd)
{
   ...
   // lws_set_log_level(LLL_DEBUG | LLL_INFO | LLL_NOTICE | LLL_WARN | LLL_ERR, NULL);
    ...
    if (!packet_callback) AP_Init_Generic();
...
void AP_SetPacketCallback(void (*callback)(json_t*)) {
    packet_callback = callback;
}

bool AP_SendPacket(json_t* packet) {
    bool queued = false;
    AP_LOCK();
    if (connected && !g_atomic_int_get(&shutting_down) && outgoing_queue &&
        outgoing_queue->length < 1024 && json_is_array(packet)) {
        g_queue_push_tail(outgoing_queue, json_incref(packet));
        queued = true;
    }
    AP_UNLOCK();
    if (queued) AP_SendWeb();
    return queued;
}

void AP_SetTLS(bool enabled) {
    AP_LOCK();
    isSSL = enabled;
    AP_UNLOCK();
}
