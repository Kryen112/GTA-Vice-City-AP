...
// GTA VC changes, randomcodegen, 2026-09-08 (LGPL-2.1-only): optional complete
// packet transport. Set callbacks before AP_Init; clear only after AP_Shutdown.
// The caller owns protocol state; callbacks run on IXWebSocket's worker thread.
void AP_SetPacketCallback(std::function<void(const std::string&)> callback);
void AP_SetTransportCallback(std::function<void(bool)> callback);
