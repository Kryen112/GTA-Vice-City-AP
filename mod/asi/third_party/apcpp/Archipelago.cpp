...
// GTA VC changes, randomcodegen, 2026-09-08 (LGPL-2.1-only).
static std::function<void(const std::string&)> packetCallback;
static std::function<void(bool)> transportCallback;
void AP_SetPacketCallback(std::function<void(const std::string&)> callback) { packetCallback = std::move(callback); }
void AP_SetTransportCallback(std::function<void(bool)> callback) { transportCallback = std::move(callback); }
bool AP_SendPacket(const std::string& packet) {
    return webSocket.getReadyState() == ix::ReadyState::Open && webSocket.send(packet).success;
}
Json::Reader reader;
...
void AP_Init(const char* ip, const char* game, const char* player_name, const char* passwd) {
    ...
    //Connect to server
    ix::initNetSystem();
    // Honor explicit ws/wss; never downgrade a TLS connection after an error.
    webSocket.setUrl(ap_ip.find("://") == std::string::npos ? "wss://" + ap_ip : ap_ip);
    webSocket.setOnMessageCallback([](const ix::WebSocketMessagePtr& msg)
        {
            if (packetCallback) {
                if (msg->type == ix::WebSocketMessageType::Message) packetCallback(msg->str);
                else if (transportCallback && msg->type == ix::WebSocketMessageType::Open) transportCallback(true);
                else if (transportCallback && (msg->type == ix::WebSocketMessageType::Close ||
                                              msg->type == ix::WebSocketMessageType::Error)) transportCallback(false);
                return;
            }
    ...
    // Packet mode does not use APCpp's data cache, and must not create a cache
    // in the game's potentially read-only working directory.
    if (!packetCallback) AP_Init_Generic();
}
