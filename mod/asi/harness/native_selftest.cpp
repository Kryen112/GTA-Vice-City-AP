...
int main(int argc, char** argv) {
  ...
  input_history.Recall(true, recalled, recalled_cursor);
  assert(recalled == L"draft" && recalled_cursor == 2);
  input_history.Add(L"first");
  input_history.Add(L"second");
  input_history.Recall(true, recalled, recalled_cursor);
  assert(recalled == L"second" && recalled_cursor == 6);
  input_history.Recall(true, recalled, recalled_cursor);
  input_history.Recall(true, recalled, recalled_cursor);
  assert(recalled == L"first");
  recalled = L"edited";
  input_history.Recall(false, recalled, recalled_cursor);
  assert(recalled == L"second");
  input_history.Recall(false, recalled, recalled_cursor);
  assert(recalled == L"draft" && recalled_cursor == 5);
  input_history.Recall(false, recalled, recalled_cursor);
  assert(recalled == L"draft");
  for (int i = 0; i < 101; ++i) input_history.Add(std::to_wstring(i));
  for (int i = 0; i < 110; ++i) input_history.Recall(true, recalled, recalled_cursor);
  assert(recalled == L"1");
  input_history.Add(L"submitted edit");
  input_history.Recall(true, recalled, recalled_cursor);
  assert(recalled == L"submitted edit");
